import { Button, Card, Col, message, Modal, Popconfirm, Popover, Row, Statistic, Tooltip } from "antd";
...
import Importer from "../../../components/CSVImporter";
import { getAdminGitfilesStatus, postAdminGitfilesManual, postAdminGitfilesStart, postAdminGitfilesStop } from "@/services/csapi/api0";

function FileImporter() {
  const [dialog, setDialog] = useState(false);

  return <>
    <Button onClick={() => setDialog(true)}>导入文件</Button>
    <Modal open={dialog} width={640} destroyOnHidden title="导入文件" onCancel={() => {
      setDialog(false)
    }} footer={null}>
      <div className="pt-4 h-96">
        <Importer service={async (l) => {
...
function RunningTask({ t }: { t: API.RunningTaskDTO }) {
...
export default function GitFileSummary() {
  ...
  const { data, error, loading, run, runAsync } = useRequest<API.GitFileStatusResp, never[]>(async () => {

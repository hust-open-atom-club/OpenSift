// PyPI enumerator for Python packages
package enumerator
...
// Get the best URL for a PyPI package
func GetBestPypiUrl(info *pypi.Info) string {
	if info.HomePage != "" {
		return info.HomePage
	}
	if info.ProjectUrls.Source != "" {
		return info.ProjectUrls.Source
	}
	return ""
}

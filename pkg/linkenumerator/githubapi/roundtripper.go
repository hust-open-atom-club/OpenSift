...
func NewRetryRoundTripper(rt http.RoundTripper, logger logger.AppLogger) http.RoundTripper {
...
func (s *strategies) ServerError(r *http.Response) (retry.RetryStrategy, error) {
	...
	logger := s.logger
	logger.Warn("5xx: detected", r.Request.RequestURI, r.Status)
...
func (s *strategies) ServerError400(r *http.Response) (retry.RetryStrategy, error) {
	...
	logger := s.logger
	logger.Warn("400: bad request detected", r.Request.URL, r.Status)
...
func (s *strategies) SecondaryRateLimit(r *http.Response) (retry.RetryStrategy, error) {
	...
	logger := s.logger
	logger.Warn("403: forbidden detected", r.Request.RequestURI, r.Status)
	errorResponse := &github.ErrorResponse{Response: r}
	data, err := io.ReadAll(r.Body)
	r.Body.Close()
	r.Body = io.NopCloser(bytes.NewBuffer(data))
	if err != nil || data == nil {
		logger.Warn("ReadAll failed", err, data)
		return retry.NoRetry, err
	}
	...
	logger.Warn("Error response data", errorResponse.DocumentationURL, errorResponse.Message)

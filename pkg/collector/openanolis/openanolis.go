...
func (cc *OpenAnolisCollector) Collect(outputPath string) {
	adc := storage.GetDefaultAppDatabaseContext()
	data := cc.GetPackageInfo(collector.OpenAnolisURL)
	cc.ParseInfo(data)
	cc.GetDep()
	cc.PageRank(0.85, 20)
	cc.GetDepCount()
...
func (cc *OpenAnolisCollector) ParseInfo(data string) error {
...
func NewOpenAnolisCollector() *OpenAnolisCollector {
	return &OpenAnolisCollector{
		CollecterInterface: collector.NewCollector(repository.OpenAnolis, repository.DistPackageTablePrefix("openanolis")),
	}
}
